...
static ssize_t probe_splice (int, loff_t *, int, loff_t *, size_t,
                             unsigned int);
#endif
...
static ssize_t
probe_readv (int fd, const struct iovec *iov, int count)
{
...
static ssize_t
probe_writev (int fd, const struct iovec *iov, int count)
{
...
#ifdef ENABLE_IO_SPLICE_SYSCALL
static ssize_t
probe_splice (int in, loff_t *off_in, int out, loff_t *off_out, size_t length,
              unsigned int flags)
{
    (void)out;
    (void)off_in;
...
static void
record (size_t received, size_t sent, void *data)
{
...
static int
stop_after_loop (HevTaskYieldType type, void *data)
{
    (void)type;
    assert (data == &calls);
    return 1;
...
main (void)
{
    ...
    directional = 1;
    calls = 0;
    hev_task_io_splice_with_stats (10, 10, 20, 20, 64, stop_after_loop, &calls,
                                   record, &callback_count);
    assert (callback_count == 1 && callback_received == 7 &&
            callback_sent == 3);
    ...
    puts (
        "PASS: external-side callback precedes cancellation; legacy API preserves I/O call count");
#endif
    return 0;
}
