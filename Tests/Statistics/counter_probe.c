...
enum { THREADS = 8, REPEATS = 100000 };

static void *writer(void *arg)
{
    size_t n = (size_t)arg;
    for (int i = 0; i < REPEATS; i++)
        hev_socks5_transfer_add((size_t)amount + n, (size_t)amount + 2 * n, NULL);
...
int main(void)
{
    ...
    printf("PASS: 8 writers, 800000 updates, concurrent monotonic reads; totals=%" PRIu64 "/%" PRIu64 "\n", in, out);
    return 0;
}
