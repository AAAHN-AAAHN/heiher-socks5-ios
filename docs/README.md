...
`release/integrated` is the complete application. `main` is the unmodified upstream
fork baseline, not the release branch. Read the limitations before exposing a proxy
...
| --- | --- | --- |
| `main` | Exact original `83d350d28d6a31e7730cfa74b49f58d2af642397` | None |
| `feature/udp-compat` | Both Darwin UDP compatibility repairs | Pristine main |
| `feature/traffic-statistics` | TCP/UDP byte measurement, C API, Statistics screen | UDP compatibility |
| `feature/background` | Continuous location, silent audio and recovery, persistent switches | Pristine main |
| `feature/settings` | Whole-model JSON, interchange, saved execution/tab state, lifecycle fixes | Pristine main |
| `feature/app-icon` | Existing icon asset and catalog | Pristine main |
...
remain available. Main is restored only after every required check succeeds, with
